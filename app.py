...
translations = {
    "English": {
        ...
        "input_label": "🆔 Employee Name (NAME / الاسم):",
        ...
        "dashboard_title": "📊 Employee Attendance & Absence Records",
    ...
    "العربية": {
        ...
        "subtitle": "🆔 الرجاء إدخال اسم الموظف للاطلاع على سجل الغياب والحضور.",
        ...
        "input_label": "🆔 اسم الموظف (NAME / الاسم):",
        ...
        "dashboard_title": "📊 سجل حضور وغياب الموظف",
...
def get_employee_attendance_records(df, employee_name):
    ...
    for _, row in emp_rows.iterrows():
        ...
        if is_absent and bsent_val.lower() in ["", "nan", "none", "0", "-"]:
            bsent_val = "غياب / Absent"
    ...
    return total_absences, attendance_records
...
if not st.session_state.admin_logged_in:
    ...
    if uploaded_file is not None:
        try:
            ...
            if os.path.exists(SHARED_FILE):
                ...
                name_c_old = next((c for c in df_old.columns if c.upper() == "NAME" or any(k in c.lower() for k in ["الاسم", "اسم", "name"])), None)
            ...
            name_c_up = next((c for c in df_upload.columns if c.upper() == "NAME" or any(k in c.lower() for k in ["الاسم", "اسم", "name"])), None)
            pass_col = []
    ...
    if os.path.exists(SHARED_FILE):
        ...
        if df_admin is not None:
            name_c_adm = next((c for c in df_admin.columns if c.upper() == "NAME" or any(k in c.lower() for k in ["الاسم", "اسم", "name"])), None)
...
# ====================================================================
# EMPLOYEE ATTENDANCE & ABSENCE PORTAL VIEW
# ====================================================================

if st.session_state.get("logged_in_user"):
    df_verify = load_excel_df()
    user_exists = False
    name_c_ver = next((c for c in df_verify.columns if c.upper() == "NAME" or any(k in c.lower() for k in ["الاسم", "اسم", "name"])), None) if df_verify is not None else None
    ...
    total_absences, attendance_list = get_employee_attendance_records(df_verify, st.session_state.get("logged_in_user"))
    ...
    try:
        ...
        if df is None:
            ...
            name_c_main = next((c for c in df.columns if c.upper() == "NAME" or any(k in c.lower() for k in ["الاسم", "اسم", "name"])), None)
